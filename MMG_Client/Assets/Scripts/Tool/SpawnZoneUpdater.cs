    ...
    public class SpawnZoneUpdater
    {
        public static void UpdateFromScene(MapDataSet mapDataSet)
        {
            ...
            foreach (var zone in zones)
            {
                ...
                if (zone.spawnType == SpawnType.Player)
                {
                ...
                else if (zone.spawnType == SpawnType.Monster)
                {
                    var existing = mapData.MonsterSpawnPoints.Find(sp => sp.SpawnPointId == zone.Id);

                    // zone.spawnMonsterData → MonsterSpawnInfo 리스트 변환
                    List<MonsterSpawnInfo> monsterList = new();
                    Debug.Log($"[SpawnZoneUpdater] spawnMonsterData의 갯수 {zone.spawnMonsterData.Count}");
                    foreach (var sm in zone.spawnMonsterData)
                    {
                        ...
                        Debug.Log($"[SpawnZoneUpdater] MonsterId = {sm.monsterData.MonsterId}, Count = {sm.SpawnCount}");
